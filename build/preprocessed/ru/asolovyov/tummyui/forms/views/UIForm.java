...
public class UIForm extends Form implements ItemStateListener, CommandListener, UINavigatable {
        ...
        isVisible.removeDuplicates().sink(new Sink() {
            protected void onValue(Object value) {
                if (isVisible.getBoolean()) {
                    UIEnvironment.midlet().getDisplay().setCurrent(alert);
                }
            }
        });
    ...
    public UIForm navigationCommand(
        ...
        UIForm form = new UIForm(new Str(formTitle), new UIItem[]{ content });
        return this.navigationCommand(new Str(linkTitle), new Str(backTitle), form);
    }
    ...
    public UIForm navigationCommand(
            String linkTitle,
            UINavigatable navigatable) {
        return this.navigationCommand(new Str(linkTitle), this.titleBinding, navigatable);
    }
    ...
    public UIForm navigationCommand(
        ...
        return this.navigationCommand(new Str(linkTitle), new Str(backTitle), navigatable);
    }
    ...
    public UIForm navigationCommand(
        ...
        navigatable.backCommand(new UICommand(backTitle, Command.BACK, Integer.MIN_VALUE, new UICommand.Handler() {
            public void handle() {
                UIEnvironment.midlet().getDisplay().setCurrent(UIForm.this);
            }
        }));
        
        return this.command(new UICommand(linkTitle, new UICommand.Handler() {
            public void handle() {
                UIEnvironment.midlet().getDisplay().setCurrent(content);
            }
        }));
    }
